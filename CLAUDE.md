...
- ✅ **Kompresi foto otomatis sebelum upload (`js/image.js`)** — satu helper `compressImage()` dipakai di SEMUA titik upload (desain, progress, bukti bayar, chat customer, chat internal, chat vendor dua sisi, inbox, progress vendor). Sisi terpanjang maks 1920px + JPEG 85% — tulisan tetap terbaca, ukuran turun ~10x. Skip jika file < 400KB & tidak oversize; GIF/SVG/PDF tidak disentuh; HEIC yang tak bisa di-decode → upload asli; PNG transparan diberi background putih. Standalone-safe (tanpa import Firebase).
- ✅ **Script kompres foto lama (`tools/compress-storage.mjs`)** — jalankan sekali di Google Cloud Shell untuk mengecilkan foto yang terlanjur diupload besar. Default simulasi; `--apply` menimpa file di path yang sama dengan token download yang sama (URL di Firestore tetap valid). PDF/GIF/SVG tidak disentuh.
